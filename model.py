...
def calculate_rest_days(games_df, all_games_df):
    """
    Calculate days since last game for each team using all D1 games.
    Returns games_df with home_rest and away_rest columns added.
    """
    # Ensure dates are datetime
    all_games_df = all_games_df.copy()
    all_games_df['date'] = pd.to_datetime(all_games_df['date'])
    
    # Create a list of all games per team (both home and away appearances)
    team_games = []
    ...
    # For each team, calculate days since previous game
    ...
    def get_rest(row, team_col):
        ...
        # Try converting team_id to different types
...
def prepare_features(games, torvik, all_games, acc_team_ids):
    """
    Merge Torvik ratings for home and away teams, then create features.
    
    For each game, we join the Torvik stats as-of the game date for both teams.
    Rest days are calculated from all_games.csv for accurate values.
    """
    # Calculate rest days from comprehensive all_games data
    games = calculate_rest_days(games, all_games)
    
    # Ensure date columns are strings in same format
    games = games.copy()
    torvik = torvik.copy()
    games['date'] = pd.to_datetime(games['date']).dt.strftime('%Y-%m-%d')
    torvik['date'] = pd.to_datetime(torvik['date']).dt.strftime('%Y-%m-%d')
    # Use prior-day Torvik snapshot to avoid same-day leakage.
    games['as_of_date'] = (
        pd.to_datetime(games['date']) - pd.Timedelta(days=1)
    ).dt.strftime('%Y-%m-%d')
    
    # Stats needed for baseline + block2 + block7 features.
    stat_cols = [
        'adj_o',
        'adj_d',
        'efg',
    ...
    # Context - use rest calculated from all_games
    # Impute missing rest values with the mean (for first games of season)
    ...
    # is_neutral: 1 = neutral site (no home advantage), 0 = home team has home court advantage
    df['is_neutral'] = df['neutral_site'].astype(int)

    # Block 7: rest & congestion (Torvik-provided versions, with safe fallbacks).
...
def build_model(df, feature_cols, torvik, target='margin', use_fixed_cutoff=True):
    ...
    Uses chronological split to respect time ordering of games.
    """
    ...
    if use_fixed_cutoff:
        # Use Torvik values as-of the training cutoff date for all future games
        ...
        # Recompute fatigue_index_diff after fixed-cutoff replacements.
        if (
            future_mask.any()
        ...
        # Drop any rows that now have missing features
    ...
    # Compare to baseline (just using spread)
...
def main():
    ...
    # Train only on games where both teams appear in Torvik ratings
